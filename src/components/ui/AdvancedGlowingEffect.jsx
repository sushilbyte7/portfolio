...
const InactiveBorder = styled.div`
  pointer-events: none;
  position: absolute;
  inset: -1px;
  border-radius: inherit;
  border: 1px solid ${({ variant }) => variant === 'white' ? 'white' : 'transparent'};
  opacity: ${({ glow, disabled }) => (glow && disabled) ? '1' : '0'};
...
const GlowContainer = styled.div`
  ...
  opacity: 1;
  transition: opacity 0.3s ease;
  filter: ${({ blur }) => blur > 0 ? `blur(var(--blur))` : 'none'};
...
const GlowElement = styled.div`
  ...
  &::after {
    ...
    /* Mask properties for the glow effect */
